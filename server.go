...
func (s *Server) PlayerHandler(w http.ResponseWriter, r *http.Request) {
	...
	path := r.URL.Path
	re := regexp.MustCompile(".mkv$") // Compile the regular expression
	audioPath := re.ReplaceAllString(path, ".mp3") // Replace all occurrences
	tmplData := TemplateData{
		StaticVersion: s.staticVersion,
		VideoURL:      fmt.Sprintf("/video/%s", path),
		AudioURL:      fmt.Sprintf("/audio/%s", audioPath),
	}
...
func (s *Server) AudioHandler(w http.ResponseWriter, r *http.Request) {
	// Open Audio File
	audioFile, err := openFile(r.URL.Path)
...
func StartServer(addr string) error {
	...
	mux.Handle("/audio/", http.StripPrefix("/audio/", (NormalizePath(s.AudioHandler))))
